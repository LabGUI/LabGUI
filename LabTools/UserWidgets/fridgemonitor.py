...
Fridge Monitor for Faraday Fridge, based on labgui Faraday Fridge Monitor from GervaisLabs, updated to Python3/PyQt5
"""
...
LAKESHORE_ALLOWED = ['LS370', 'LS370_janis']
CMN_ALLOWED = ['HP426B']
...
class FridgeMonitorWindow(QtGui.QMainWindow, ui_fridgemonitor.Ui_FridgeMonitorWindow):
   ...
   # it may be adaptable for other people's fridges... 
    ...
    def __init__(self, parent=None):
        ...
        self.htrRangeComboBox.currentIndexChanged.connect(self.set_htr_range)
...
#        self.lakeshore = LS370_janis.Instrument('GPIB0::12', self.debug)
...
#        self.pirani = MKS146C.Instrument("COM8", self.debug)
        ...
        for i in ['startStopButton','autoScaleButton','manualScaleButton','cancelButton','updateButton','updatePIDButton','radioButton_PID','radioButton_Man']:
        ...
        try:
            ...
            if name == 'CMN':
                ...
                if self.PID_mode:
                    ...
                    self.lakeshore.set_heater_range(5)
                    ...
                    output = min (100.0, max(0.0, proportional_htr + integral_htr))
                    logging.info( ("I would set the output to: " + str(output)) )
                    self.lakeshore.set_heater_output(output)

                #get heater setting and include it in the data line                                                                                                                                                                                                                                                         date textbox
                heater_percent = self.lakeshore.get_heater_output()
                self.data_string = self.data_string + "     " + str(heater_percent)

                heater_range = self.lakeshore.get_heater_range()
                self.data_string = self.data_string + "     " + str(heater_range)
                #update only if user isn't currently trying to enter a value
                if not self.htr_changed:
                    self.htrOutputLineEdit.setText(str(heater_percent))
                    self.htrRangeComboBox.setCurrentIndex(heater_range)
                ...
                self.plot_widgets[self.channels['CMN-L']['subplot']].add_point(self.get_time(), data*1e3, T)
...
#            self.pirani = MKS146C.Instrument('COM8', debug = True) #'/dev/ttyUSB0'
    ...
    def set_htr_range(self):
        if self.htrRangeComboBox.currentIndex() != self.lakeshore.get_heater_range():
            self.htrRangeComboBox.setStyleSheet("color: rgb(255, 0, 0);")
            self.htr_changed = True     
        
        
    def update_htr_range(self, rg):
        """triggered by a signal from the FridgeServer"""
        rg = int(rg)
        if rg <= 8:
            self.htrRangeComboBox.setCurrentIndex(rg)
            self.lakeshore.set_heater_output(rg)
    ...
    def on_updateButton_clicked(self):
        ...
        try:
            ...
            if val < 0 or val > 100:
                self.htrOutputLineEdit.setText(str(self.lakeshore.get_heater_output()))
            else:
                logging.debug ("setting to " + str(val))
                self.lakeshore.set_heater_output (val)
        except ValueError:
            self.htrOutputLineEdit.setText(str(self.lakeshore.get_heater_output()))
        
        self.lakeshore.set_heater_range(self.htrRangeComboBox.currentIndex())
    ...
    def update_heater_display(self):
        ...
        self.htrRangeComboBox.setStyleSheet("color: rgb(0, 0, 0);")        
        self.htrOutputLineEdit.setText(str(self.lakeshore.get_heater_output()))
        self.htrRangeComboBox.setCurrentIndex(self.lakeshore.get_heater_range())
    ...
    def ready_start(self):
        try:
            if self.parent is not None:
                ...
                self.lakeshore = self.CMN = self.pirani = None
                for instr in self.sanitized_list: # find required devices
                    if self.lakeshore is None or CHOOSE_FIRST_DEVICE:
                        if instr[0] in LAKESHORE_ALLOWED:
                            self.lakeshore = instr[2]
                ...
                flag = True
                if self.lakeshore is None:
                    if DEBUG:
                        self.lakeshore = FakeInstr()  # LS370_janis.Instrument('name', debug=True)
                    else:
                        logging.info("Missing lakeshore device.")
                        flag = False
                if self.CMN is None:
                    if DEBUG:
                        self.CMN = FakeInstr()  # HP4263B.Instrument('name', debug=True)
...
def add_widget_into_main(parent):
    ...
    FMDockWidget = QtGui.QDockWidget("Fridge Monitor", parent)
    FMDockWidget.setObjectName("FridgeMonitorWidget")
    FMDockWidget.setAllowedAreas(
        QtCore.Qt.LeftDockWidgetArea | QtCore.Qt.RightDockWidgetArea)

    # fill the dictionnary with the widgets added into LabGuiMain
    parent.widgets['FridgeMonitorWidget'] = mywidget

    FMDockWidget.setWidget(mywidget)
    parent.addDockWidget(QtCore.Qt.RightDockWidgetArea, FMDockWidget)

    # Enable the toggle view action
    parent.windowMenu.addAction(FMDockWidget.toggleViewAction())
    #FMDockWidget.hide()
