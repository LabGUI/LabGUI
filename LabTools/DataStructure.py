    ...
    def __init__(self, data=None, labels=None, fname=None,
        ...
        if not fname == None:
            ...
            if self.ncols == self.nlabels:
                ...
                logging.error("There is %i labels for %i columns in the data matrix" % (
                    self.nlabels, self.ncols))
        ...
        if np.size(inargs) == 2:
            ...
            if label in self.labels:
                ...
                if isinstance(label, int) or isinstance(label, slice):
                    ...
                    logging.warning(
                        "The indexing variable %s doesn't have a match in the possible indexes, it is therefore ignored" % (label))
                    label = None
...
def test_labels_class_LabeledData(labels=["a", "b", "c"]):
    ...
    if (mydat[0:2] == np.array([[1, 2, 3], [4, 5, 6]])).all():
        logging.info(
            "the indexing with a slice for the first argument alone works")
        test_passed += 1
    else:
        logging.warning(
            "the indexing with a slice for the first argument alone fails")
    ...
    if (mydat[:, labels[1]] == np.array([2, 5, 8])).all():
        logging.info(
            "the indexing by label in second argument with a slice for the first argument works")
        test_passed += 1
    else:
        logging.warning(
            "the indexing by label in second argument with a slice for the first argument fails")

    if mydat[1, labels[1]] == 5:
        logging.info(
            "the indexing by label in second argument with a index for the first argument works")
        test_passed += 1
    else:
        logging.warning(
            "the indexing by label in second argument with a index for the first argument fails")

    if (mydat[-1] == np.array([7, 8, 9])).all():
        logging.info(
            "the negative indexing by index in first argument alone works")
        test_passed += 1
    else:
        logging.warning(
            "the negative indexing by index in first argument alone fails")
    ...
    if mydat.nlines == 2 * nlines:
        logging.info(
            "the merge function returns an object with the correct number of lines")
        test_passed += 1
    else:
        logging.warning(
            "the merge function returns an object with the incorrect number of lines")

    if mydat.ncols == ncolumns:
        logging.info(
            "the merge function returns an object with the correct number of columns")
        test_passed += 1
    else:
        logging.warning(
            "the merge function returns an object with the incorrect number of columns")

    if mydat.labels == labels:
        logging.info(
            "the merge function returns an object with the correct labels")
        test_passed += 1
    else:
        logging.warning(
            "the merge function returns an object with the incorrect labels")
