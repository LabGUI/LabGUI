...
#                self.line_layout.addWidget(self.bt_connecthub, 0,3,1,1)
                ...
                self.bt_remove_last = QtGui.QPushButton(
                    "Remove last item", self)
                self.bt_remove_last.setEnabled(True)
                self.bt_remove_last.clicked.connect(
                    self.bt_remove_last_clicked)
...
#        print 'line created'
        ...
        if USE_PYQT5:
            ...
            self.connect(new_line, SIGNAL(
                "colorChanged()"), self.update_colors)
...
def test_load_settings():
    ...
    ex.load_settings(
        "C:\\Users\\pfduc\\Documents\\labgui_github\\settings\\test_settings.set")
