...
class FittingWidget(QtGui.QWidget):
    ...
    # (pan,zoom,selection)
    fit_selection_parameters = {"X_line_idx": 0,
                                "Fit_line_idx": 1, "limits": [], "mode": 1}
    ...
    def data_type_changed(self, data_type=None):
        ...
        if data_type is None:
            ...
            if self.data_type in ['Live', 'Past']:
                ...
                logging.error(
                    "The data type passed in argument is not of the right format")
...
#            print "replacing", np.shape(self.data_array), "by", np.shape(data)
        ...
        if self.state:
            ...
            if not (X == VOID_NPARRAY and Y == VOID_NPARRAY):
                ...
                # performs the fit
                fitp_val, cov = ad.fit_nonlinear(
                    X, Y, self.fit_func, guess_param)
    ...
    def get_lineedit_value(self, id_label):
        ...
        if np.size(widget) == 1:
            ...
            msg = "There is more than one LineEdit named %s for the fonction %s" % (
                id_label, self.fit_func.func_name)
    ...
    def get_checkbox_state(self, id_label):
        ...
        if np.size(widget) == 1:
            ...
            msg = "There is more than one checkbox named %s for the fonction %s" % (
                id_label, self.fit_func.func_name)
            raise(ValueError, msg)
        return answer
